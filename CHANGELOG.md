...
### Исправлено
- **QUIC blackhole-fix переведён с whitelist на инверсию (точечный reject Google)**: вчерашний `reject UDP/443 from LAN` ломал любой сервис с активным HTTP/3 — Telemost (встреча + чат), приложение Сбера, потенциально много чего ещё. Whitelist пришлось бы бесконечно расширять под каждый банк/сервис. Перешли на обратную логику: вместо «реджектим всё, кроме whitelist» — реджектим **только Google AS15169** (откуда родом YouTube QUIC blackhole) + публичные DoH-резолверы (1.1.1.1, 1.0.0.1, 9.9.9.9, 8.8.8.8). Set `google_quic_reject` в `net-enforcer/main.py`. Yandex/Сбер/Telegram/всё остальное QUIC проходит без вмешательства, YouTube blackhole-fix продолжает работать.
