...
### Изменено
- **Синхронный regen Mihomo config при смене режима устройства через UI** (`web-orchestrator/main.py`, `docker-compose.yml`, `web-orchestrator/Dockerfile`, `tunnel-provider/entrypoint.sh`). Инцидент 2026-07-02 (retail Hisense TV + iPad): пользователь переключил iPad `.173` в UI из global в smart, `devices.json` обновился, но в Mihomo `config.yaml` осталось старое правило `SRC-IP-CIDR,192.168.1.173/32,auto` (весь трафик через VPN) — sub-rule для smart-режима **не сгенерировался**. Rutube в браузере на iPad → через шведский узел → блок. Причина: inotify+30-сек-debounce в `tunnel-provider/entrypoint.sh` иногда пропускает событие (atomic write не всегда триггерит `close_write`, batch правок схлопываются в один regen с последним состоянием). Правки: (1) web-orchestrator получает доступ к `docker.sock` (volume-mount в docker-compose.yml) и `docker-cli` (Dockerfile), (2) новая функция `_regen_tunnel_config_sync` вызывает `docker exec gsg-tunnel python3 generate_config.py` + `PUT /configs?force=true` из handler'а `update_device` и других write-endpoints, (3) старая `.reload_singbox` маркер-файл + inotify оставлены как fallback safety net на случай если docker exec недоступен, (4) уменьшен debounce с 30→5с — быстрее реакция при потере event.
- **`DEFAULT_DIRECT_DOMAINS` расширен на RU-стриминги** (`tunnel-provider/generate_config.py`). Инцидент 2026-07-02 (retail Hisense TV): Кинопоиск/Rutube/Okko/etc. попадали в auto-group через MATCH → VPN-узел → сервисы отвечали «не в вашем регионе». Полагаться только на конечное `DOMAIN-SUFFIX,ru,DIRECT` в chain нельзя — короткие соединения могут проскакивать до этого правила через sniffer race. Добавил явно: kinopoisk.ru+kp.cdn.yandex.net, okko.tv/okko.sport, ivi.ru/ivi.tv, wink.ru, premier.one, more.tv, start.ru, smotrim.ru, rutube.ru, 1tv.ru, ntv.ru, vgtrk.ru, kion.ru, kino.mail.ru, video.mail.ru, videocdn.tv + Yandex-CDN'ы (yastatic.net, mds.yandex.net, strm.yandex.net, ott-widget.yandex.net, vh.yandex.ru).
- **`gsg-warmer.sh` — HEAD → GET с Range:bytes=0-4095** (`gsg-warmer.sh`). Замена `curl --head` на `curl --http1.1 -H "Range: bytes=0-4095" + Mozilla User-Agent + Accept-Language: ru`. Anti-DDoS у банков (особенно tinkoff/trbcdn/sber) воспринимает HEAD как «синтетический запрос» и быстро (≤4 мин) expir'ит src-port из whitelist. Полноценный GET с body-чтением и реалистичным User-Agent держит TCP-сессию дольше (TLS+HTTP+data) и удлинняет whitelist. Эффект: пять прогонов после смены — 20/24 ok вместо 14-18/24 на старом HEAD-варианте. Упорные остаются (`trbcdn`/жёсткий Tilda WAF), но банков теперь пробивает стабильнее. Связано с [[BACKLOG#0.6]] (cold-start lag).
- **Mihomo DNS — переход на DoH (Cloudflare + AdGuard)** (`tunnel-provider/generate_config.py`). РКН добавляет домены в DNS-фильтр (`*.hunyuanglobal.com → 0.0.0.1` подтверждено 2026-05-28 для Tencent Hunyuan 3D) — UDP/53 ответы перехватываются провайдером и подменяются на sinkhole. DoH через HTTPS/443 это обходит. **Подводный камень:** **Google DoH автоматически добавляет EDNS-Client-Subnet из IP клиента** (наш end0 = RU subnet) → китайские authoritative-серверы возвращают `0.0.0.1` по ECS-RU. Поэтому в `nameserver` только Cloudflare (`https://cloudflare-dns.com/dns-query`) и AdGuard (`https://dns.adguard-dns.com/dns-query`) — оба без ECS. `default-nameserver` остался plain `1.1.1.1/8.8.8.8` для bootstrap (резолв самих DoH-хостов при старте Mihomo). Также убрал `geosite:cn` из `nameserver-policy` — раньше китайские домены шли через plain `77.88.8.8` (Yandex), который тоже фильтровался РКН. Теперь они идут через основной DoH-цепочкой. RU-домены (`+.ru/+.рф/+.su`) остались на plain Yandex DNS — РКН их не блокирует, plain UDP быстрее.

### Добавлено
- **`gsg-warmer.service` + `.timer` — каждые 4 минуты прогревает src-port для anti-DDoS upstream российских сервисов** (`gsg-warmer.sh`, `gsg-warmer.service`, `gsg-warmer.timer`, `install.sh`, `update-watcher.sh`). Решает класс жалоб «сайт открывается на 3-4 ретрае через минуту» (komanda.fit 21.05, tinkoff.ru 22.05, Сбер/Яндекс.Карты 23.05, Парковки Москвы 23.05, Т-банк 24.05, Авито 24.05, market.yandex.ru 24.05 — 7 случаев за 3 дня). Корневая причина (см. `Research/2026-05-24-cold-start-lag-results`): серверный grey-list anti-DDoS на стороне Tinkoff/Gosuslugi/Tilda/etc. дропает первые SYN от «холодных» NAT-flow → 7-15с до whitelist. Pcap подтвердил: внутри GSG задержка LAN-SYN → WAN-SYN всегда <200μs; для komanda.fit сервер ack-нул именно 8-й SYN. Warmer делает периодический HEAD-запрос к 24 топовым RU-доменам (банки, госуслуги, маркеты, контент) — поддерживает src-port в whitelist upstream. Замер: до warmer'а `tinkoff.ru` 15с timeout с MacBook, после — 11мс. CPU/трафик незначительные (~24 HEAD-запроса × 4 минуты). Список доменов в `gsg-warmer.sh` (массив `DOMAINS`).

### Исправлено
- **`_detect_lan_ip` — retry с backoff + fallback на `/proc/net/route`** (`web-orchestrator/main.py`). Инцидент 2026-07-02 у retail-пользователя (RaspberryPi 4): при cold-boot после установки Docker стартует контейнеры **до** того как NetworkManager поднял `eth0` и получил default route. Мой код от 18.05 (`s.connect(("1.1.1.1", 53))` для получения src-IP через `getsockname`) бросал `OSError: [Errno 101] Network is unreachable` → web-orchestrator падал → Docker `restart:always` ретраил → после 2-3 попыток docker переходил в exponential backoff → пользователь видел «контейнеры не запускаются». Ручной `docker compose up -d` через минуту после boot (сеть уже готова) — всё стартовало. Теперь: (1) 6 попыток UDP-connect с задержкой 2с = до 12с ожидания сети; (2) fallback на чтение `/proc/net/route` + `SIOCGIFADDR` через `fcntl.ioctl` — работает даже без интернета если есть default gateway. Ретейл-пользователь пропускает симптом полностью.
- **`install.sh` — `wait_containers_up` после `docker compose up -d`**. Ждёт до 60с что все 4 контейнера в состоянии `running`. Если нет — делает `docker compose up -d --force-recreate` (к этому моменту сеть уже готова). Страховка от того же класса гонок cold-boot.
- **`uninstall.sh` — читаем подтверждение из `/dev/tty`** при запуске через `curl … | sudo bash` (stdin занят потоком скрипта). Если tty недоступен — без подтверждения (для cron-сценариев).
- **Хардкод `10.10.1.x` убран из кода — GSG корректно работает в любой подсети** (`web-orchestrator/main.py`, `net-enforcer/main.py`). При переносе GSG в чужую сеть (10.10.2.x, 192.168.x.x) код использовал хардкод-fallback `10.10.1.139` (`GATEWAY_IP = os.getenv("GSG_GATEWAY_IP", "10.10.1.139")`); env-vars в `docker-compose.yml` для web-orchestrator не передавались — контейнер видел только дефолт. Следствия: (1) ARP-фильтр в `/api/devices` смотрел `lan_prefix = "10.10.1."` → **все устройства из 10.10.2.x отбрасывались** → UI «Устройства» был **пустой** на даче; (2) `/api/dhcp` отдавал в дефолтах `pool 10.10.1.100-200`, `dns 10.10.1.139` — утечка домашних IP в новую установку; (3) `net-enforcer/detect_lan_cidr()` fallback тоже `10.10.1.0/24`. Теперь все три места автоматически определяют LAN-IP через стандартный приём «UDP-сокет к 1.1.1.1:53 → `getsockname()`» — без зависимости от утилит/env-vars/netplan, работает с любого netns. Fail-fast если детект не сработал (никаких чужих сетей в дефолте). Инцидент `2026-05-18-gsg-dacha-deployment`.
