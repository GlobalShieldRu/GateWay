...
### Исправлено
- **Yandex Telemost не подключается к встрече («Произошла ошибка»)**: вчерашний QUIC-reject для UDP/443 ломал Yandex Browser/Telemost — Telemost активно использует HTTP/3 для сигналинга и медиа-серверов в Yandex.Cloud, и фолбэк на TCP у их клиента работает плохо («сначала страница недоступна», потом ошибка в приложении). В `net-enforcer/main.py` добавлен nft set `yandex_nets` с CIDR обоих Yandex ASN (AS13238 + AS208722 Yandex.Cloud — 18 префиксов), и правило `ip daddr @yandex_nets udp dport 443 return` стоит до reject — QUIC к Yandex проходит штатно, к остальным сервисам остаётся blackhole-fix YouTube.
