...
### Исправлено
- **Yandex Telemost не подключается к встрече и чат внутри совещания не открывается**: QUIC-reject для UDP/443 (фикс YouTube) ломал Yandex Browser/Telemost — Telemost использует HTTP/3 для сигналинга и медиа-серверов, а фолбэк на TCP у их клиента работает плохо («сначала страница недоступна», потом «Произошла ошибка» в приложении, чат циклично крутится в reconnect). В `net-enforcer/main.py` добавлен nft set `quic_bypass_nets` (24 CIDR: AS13238 Yandex основной + AS208722 Yandex.Cloud media-серверы Telemost + AS54113 Fastly CDN под yastatic.net и Telemost-чатом), и правило `ip daddr @quic_bypass_nets udp dport 443 return` стоит до reject — QUIC к этим сервисам проходит штатно, к остальным остаётся blackhole-fix для YouTube.
