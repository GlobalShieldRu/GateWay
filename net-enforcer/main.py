...
{node_set}
    # QUIC-bypass whitelist для UDP/443 — пропускается БЕЗ TPROXY и БЕЗ reject.
    # Архитектурный контекст:
    #   1. Mihomo TPROXY на этом ядре blackhole'ит UDP — ответы клиенту не доходят.
    #   2. Поэтому ниже стоит глобальный `reject UDP/443 from LAN` — браузер ловит
    #      ICMP unreachable и сразу делает TCP fallback (так чинится YouTube).
    #   3. Но некоторые приложения TCP fallback не делают (Yandex Telemost клиент,
    #      Сбер, "Моя школа"/Госуслуги мобильные приложения) — для них нужен
    #      явный bypass: их IP идёт напрямую через DIRECT с masquerade (return).
    # Содержит:
    #   - Yandex AS13238 + Yandex.Cloud AS208722 (Telemost media + чат)
    #   - Fastly AS54113 (yastatic.net + множество других CDN)
    #   - Sberbank AS35237
    #   - Telegram AS62041/AS62014/AS44907/AS59930 (видео-CDN — без bypass'а
    #     QUIC получал ICMP port-unreachable и Telegram не грузил видео)
    #   - RU гос/школа/телекомы (Минцифры 109.207.0.0/16, Ростелеком 213.59.0.0/16,
    #     Связьинвест 176.211.0.0/16, МосОблЭлектро 94.79.0.0/16, Госуслуги/ESIA)
    # TODO: заменить на динамический GeoIP RU (RIPE delegated → cron daily refresh).
    set quic_bypass_nets {{
        ...
        elements = {{
            ...
            109.207.0.0/16, 213.59.0.0/16, 176.211.0.0/16, 94.79.0.0/16,
            91.103.0.0/16, 92.255.0.0/16, 178.248.0.0/16, 217.118.0.0/16,
